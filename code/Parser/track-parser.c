...
TrackData_t *LoadTrackData (const char *file)
{
		...
		if (!JSON_GetInt(JSON_GetField(root, "num-vertices"), &(data->nVerts))
						...
						||  !JSON_GetInt(JSON_GetField(root, "num-starts"), &(data->nStarts)))
				goto error_cleanup;
		...
		// get the starts
		if ((data->nStarts < 0) || (65536 <= data->nLanes))
				goto error_cleanup;
		else {
				JSON_Array_t *starts = JSON_GetArray(JSON_GetField(root, "starts"));
				if (starts->length != data->nStarts)
						goto error_cleanup;
				data->starts = NEWVEC(Start_t, data->nStarts);
				bzero(data->starts, sizeof(Start_t)*data->nStarts);
				for (int i = 0; i < data->nStarts; i++) {
						JSON_Value_t *start = starts->elems[i];
						if ((! GetUInt16Field(start, "pos", &(data->starts[i].pos)))
										|| (! GetUInt16Field(start, "seg", &(data->starts[i].sect))))
								goto error_cleanup;
				}
		}
